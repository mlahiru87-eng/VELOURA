...
import { Play, Flame, Film, Sparkles, AlertTriangle, ShieldCheck, Heart, ArrowRight } from 'lucide-react';
...
function StreamingPortalContent() {
  ...
  // Selected video to load into the Admin editing drawer
  const [editTarget, setEditTarget] = useState<Video | null>(null);

  // Filter video collection on search and selected category
  ...
  // Split into structural lists
  const featuredVideo = filteredVideos.find((v) => v.isFeatured) || filteredVideos[0];
  const trendingVideos = filteredVideos.filter((v) => v.isTrending);
  ...
  return (
    <div className="min-h-screen bg-[#0B0B0F] text-zinc-100 flex flex-col font-sans selection:bg-gold-400 selection:text-black pb-20 md:pb-8">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-6 space-y-8">
        ...
        {/* 3. LATEST VIDEOS / EXPANSIVE ARCHIVE */}
        <section className="space-y-4">
          <div className="flex items-center justify-between border-b border-gold-500/10 pb-2.5">
            <h3 className="text-sm md:text-base font-serif font-semibold uppercase tracking-wide text-white flex items-center gap-2">
      ...
      {/* Floating Video Player cinema stage */}
      <VideoPlayerModal />
